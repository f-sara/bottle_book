...
def get_connection():
    ...
    dsn = dsn.format(host=DB_HOST, port=DB_PORT, dbname=DB_NAME, user=DB_USER, password=DB_PASS)
...
def add():
    ...
    パスワード:<input type="password" name="passwd" value="<!--passwd-->" /><br />
...
def list():
    sql = """
    select user_id, email, user_shi, user_mei \
    from book_user \
    where del = false \
    ...
    return template('user/views/list.html', rows=rows)
